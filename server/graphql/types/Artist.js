...
const Artist = new GraphQLObjectType({
  ...
  fields: () => ({
    albums: {
      ...
      resolve: (root, args, context) => grabToken().then(token => fetch(`${API_URL}/artists/${root.id}/albums`, {
        headers: {
    ...
    images: {
      ...
      resolve: (root, args, context) => {
        ...
        return root.images.slice(0, options.limit);
      },
    },
    ...
    related: {
      ...
      resolve: (root, args, context) => grabToken().then(token => fetch(`${API_URL}/artists/${root.id}/related-artists`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      })
    ...
    tracks: {
      ...
      resolve: (root, args, context) => {
        ...
        return grabToken().then(token => fetch(`${API_URL}/artists/${root.id}/top-tracks?country=${options.country}`, {
          headers: {
            Authorization: `Bearer ${token}`,
          },
        })
